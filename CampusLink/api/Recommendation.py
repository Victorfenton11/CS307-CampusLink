...
def userNameToID(df, index_of_user, id):
    ...
    if id in idList:
        ...
        idList.pop()
    return addCirclesAndPrint(id, idList)


def addCirclesAndPrint(id, idList):
    circles = pd.DataFrame(list(Circle.objects.all().values()))
    user = User.objects.get(UserID=id)
    userInterest = user.Interest
    userCircles = [circle.id for circle in user.Circles.all()]

    recommendedCircles = []

    for _, circle in circles.iterrows():
        if circle["public"] == True and circle["Name"] == userInterest:
            # check user is not already in the circle
            if not circle["id"] in userCircles:
                recommendedCircles.append(circle["id"])

    print(recommendedCircles)

    return printUsersAndCircles(idList, recommendedCircles)


def printUsersAndCircles(usersList, circlesList):
    users = User.objects.filter(UserID__in=usersList)
    user_serializer = UserSerializer(users, many=True)
    circles = Circle.objects.filter(id__in=circlesList)
    circle_serializer = CircleSerializer(circles, many=True)

    recommendations = {"users": user_serializer.data, "circles": circle_serializer.data}
